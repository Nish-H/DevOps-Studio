...
import { FileText, Search, Plus, Folder, Tag, Download, Upload, Eye, Edit2, Trash2, X, Save, Star, Archive, Clock, Filter, Grid, List, BookOpen, FileCode, Image as ImageIcon, File } from 'lucide-react';
...
**Use Case:** All network implementation projects`,
  ...
  useEffect(() => {
    if (documents.length > 0) {
      localStorage.setItem('nishen-workspace-documentation', JSON.stringify(documents));
    }
  }, [documents]);

  useEffect(() => {
    if (categories.length > 0) {
      localStorage.setItem('nishen-workspace-doc-categories', JSON.stringify(categories));
    }
  }, [categories]);
  ...
  const handleSaveDocument = () => {
    if (activeDocument) {
      ...
      setActiveDocument({ ...activeDocument, content: editContent });
      setIsEditing(false);
    }
  };
